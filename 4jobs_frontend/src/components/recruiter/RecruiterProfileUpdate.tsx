...
const RecruiterProfileUpdate: React.FC = () => {
  ...
  const validateForm = () => {
    ...
    if (!nameRegex.test(formData.name)) {
      newErrors.name =
        "Name must be 3-20 characters and contain only letters and spaces.";
      valid = false;
    }
    ...
    if (!nameRegex.test(formData.companyName)) {
      newErrors.companyName =
        "Company name must be 3-20 characters and contain only letters and spaces.";
      valid = false;
    }
    ...
    if (!locationRegex.test(formData.location)) {
      newErrors.location =
        "Location must be 3-15 characters and can contain letters, spaces, and commas.";
      valid = false;
    }
    ...
    if (formData.governmentId) {
      ...
      if (!validExtensions.test(fileName)) {
        newErrors.governmentId =
          "Government ID must be a valid jpg, jpeg, png, or pdf file.";
        valid = false;
      }
    }
    ...
    if (formData.employeeIdImage) {
      ...
      if (!validExtensions.test(fileName)) {
        newErrors.employeeIdImage =
          "Employee ID Image must be a valid jpg, jpeg, png, or pdf file.";
        valid = false;
      }
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.governmentId)
      updatedData.append("governmentId", formData.governmentId);
    if (formData.employeeIdImage)
      updatedData.append("employeeIdImage", formData.employeeIdImage);

    dispatch(
      updateProfile({ recruiterId: recruiter?.id, profileData: updatedData })
    ).then(() => {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            {errors.employeeIdImage && (
              <p className="text-red-500 text-sm">{errors.employeeIdImage}</p>
            )}
          </div>

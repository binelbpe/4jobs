...
const EditPost: React.FC = () => {
  ...
  const updatePostInBackend = useCallback(
    async (
      postData: Partial<CreatePostData>,
      postId: string,
      userId: string
    ...
    async (inputFile: File): Promise<File> => {
      ...
      try {
        ...
        const targetBitrate = Math.floor(
          (TARGET_VIDEO_SIZE * 8 * 1024 * 1024) / duration
        );
        ...
        if (pendingUpdate) {
          const updatedPostData = {
            ...pendingUpdate.postData,
            video: inputFile,
          };
          await updatePostInBackend(
            updatedPostData,
            pendingUpdate.postId,
          ...
          <div className="p-6 border-t border-purple-200">
            ...
            <div className="space-y-4 max-h-60 overflow-y-auto">
              {post.comments.map((comment: Comment) => (
                <div
                  key={comment.id}
                  className="bg-purple-50 p-3 rounded-lg flex justify-between items-start"
                >
                  <div>

...
import { fetchConversation, markMessagesAsRead, setTypingStatus, addMessage } from '../../redux/slices/messageSlice';
...
const Conversation: React.FC<ConversationProps> = ({ userId }) => {
  ...
  const conversationId = currentUser && (currentUser.id < userId ? `${currentUser.id}-${userId}` : `${userId}-${currentUser.id}`);
  const messages = useSelector((state: RootState) => state.messages.conversations[conversationId || ''] || []);
  ...
  const isTyping = useSelector((state: RootState) => {
    const typingStatus = state.messages.connections.find(conn => conn.user.id === userId)?.isTyping || false;
  ...
  const fetchConversationData = useCallback(() => {
    if (currentUser && conversationId && !hasFetchedConversation && messages.length === 0) {
      dispatch(fetchConversation({ userId1: currentUser.id, userId2: userId }));
      setHasFetchedConversation(true);
    }
  }, [currentUser, conversationId, dispatch, userId, hasFetchedConversation, messages.length]);
  ...
  useEffect(() => {
    ...
    if (unreadMessages.length > 0) {
      dispatch(markMessagesAsRead({ messageIds: unreadMessages.map((m: Message) => m.id) }));
    }
  ...
  const handleSendMessage = useCallback((e: React.FormEvent) => {
    ...
    if (newMessage.trim() && currentUser) {
      ...
      dispatch(addMessage(messageData)); // Add message to local state immediately
      sendSocketMessage(messageData); // Send message through socket
      setNewMessage('');
    }
    ...
    <div className="flex flex-col h-full">
      <div className="p-4 border-b border-gray-300">
        ...
        <div className="flex items-center text-sm text-gray-500">
          {isOnline && <span className="mr-2 w-2 h-2 bg-green-500 rounded-full inline-block"></span>}

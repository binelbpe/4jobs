...
const AdvancedJobSearch: React.FC = () => {
  ...
  const handleSearch = () => {
    ...
    const filters: JobSearchFilters = {
      ...searchParams,
      wayOfWork: searchParams.wayOfWork || undefined,
    };
  ...
  const handleSkillAdd = (inputValue: string) => {
    const newSkills = inputValue
      ...
      .filter(
        (skill) => skill.length > 0 && !searchParams.skills.includes(skill)
      );
  ...
  const handleViewDetails = async (jobId: string) => {
    try {
      const job = [...exactMatches, ...similarMatches].find(
        (j) => j._id === jobId
      );
          ...
          <Grid container spacing={isSmallScreen ? 2 : 3}>
            ...
            <Grid item xs={12} md={4}>
              <FormControl fullWidth variant="outlined">
                ...
                <FormHelperText>
                  Select the type of work arrangement
                </FormHelperText>
              </FormControl>
            </Grid>

...
const UserProfile: React.FC = () => {
  ...
  if (!user) {
    return <div className="text-center text-gray-600 mt-10">No user data available.</div>;
  }
        ...
        <div className="bg-white shadow-lg rounded-lg overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row items-center">
              ...
              <div className="md:ml-6 mt-4 md:mt-0 text-center md:text-left">
                ...
                <div className="mt-4 space-x-4">
                  <button
                    className="px-6 py-2 bg-white text-purple-600 rounded-full hover:bg-gray-100 transition duration-300 ease-in-out transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-purple-400 focus:ring-opacity-50"
                    onClick={handleUpdateProfile}
                  >
                    Update Profile
                  </button>
                  <button
                    className="px-6 py-2 bg-purple-500 text-white rounded-full hover:bg-purple-600 transition duration-300 ease-in-out transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-purple-400 focus:ring-opacity-50"
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 gap-6">
              <LongProfileDetail icon={<Info />} title="Bio" content={user?.bio || "No bio available."} />
              <LongProfileDetail icon={<User />} title="About" content={user?.about || "No details available."} />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
              <ProfileDetail icon={<Calendar />} title="Date of Birth" content={user?.dateOfBirth ? new Date(user.dateOfBirth).toLocaleDateString() : "Not specified"} />
              <ProfileDetail icon={<User />} title="Gender" content={user?.gender || "Not specified"} />
              <ProfileDetail icon={<Award />} title="Skills" content={user?.skills?.join(", ") || "No skills listed."} />
            </div>
...
const LongProfileDetail: React.FC<{ icon: React.ReactNode; title: string; content: string }> = ({ icon, title, content }) => (
...
const ProfileDetail: React.FC<{ icon: React.ReactNode; title: string; content: string }> = ({ icon, title, content }) => (
...
const CertificateCard: React.FC<{ certificate: any }> = ({ certificate }) => (
  <div className="border rounded-lg shadow-md overflow-hidden transition-transform transform hover:scale-105">
    <div className="p-4 bg-gray-50">
      <h4 className="font-semibold text-md text-gray-800">{certificate.name}</h4>
...
const ProjectCard: React.FC<{ project: any }> = ({ project }) => (
  <div className="border rounded-lg shadow-md overflow-hidden transition-transform transform hover:scale-105">
    <div className="p-4 bg-gray-50">
      ...
      <a href={project.link} target="_blank" rel="noopener noreferrer" className="text-purple-600 hover:text-purple-800 mt-2 inline-flex items-center">
...
const ExperienceCard: React.FC<{ experience: any }> = ({ experience }) => (
  <div className="border rounded-lg shadow-md overflow-hidden transition-transform transform hover:scale-105">
    <div className="p-4 bg-gray-50">
      <h4 className="font-semibold text-md text-gray-800">{experience.position}</h4>
      <p className="text-sm text-gray-600 mt-1">Company: {experience.company}</p>
      <p className="text-xs text-gray-500 mt-1">Duration: {experience.startDate} - {experience.endDate}</p>

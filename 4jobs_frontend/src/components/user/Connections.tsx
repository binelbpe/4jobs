...
const Connections: React.FC = () => {
  ...
  const handleRemove = async (connectionId: string) => {
    if (user) {
      try {
        await dispatch(removeConnection({ userId: user.id, connectionId })).unwrap();
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-xl sm:text-2xl font-bold mb-4 text-purple-800">My Network</h1>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="bg-white mb-4">
              <h2 className="text-lg sm:text-xl font-semibold mb-2 text-purple-800">Connection Requests</h2>
              {connectionRequests.length === 0 ? (
                <p className="text-xs text-gray-600">No pending connection requests.</p>
              ) : (
                <ul className="divide-y divide-gray-200">
                  {connectionRequests.map((request: ConnectionRequest) => (
                    <li key={request._id} className="py-2 flex items-center justify-between">
                      <div className="flex items-center">
                        <img src={request.requester.profileImage || '/default-avatar.png'} alt={request.requester.name} className="w-8 h-8 rounded-full object-cover" />
                        <div className="ml-1">
                          <h3 className="text-xs font-semibold text-purple-800">{request.requester.name}</h3>
                          <p className="text-xs text-gray-600">{truncateBio(request.requester.bio)}</p>
                        </div>
                      </div>
                      <div className="flex space-x-1">
                        ...
                        >
                          <FontAwesomeIcon icon={faUserCheck} className="mr-1 h-3 w-3" /> 
                        ...
                        <button 
                          ...
                          disabled={localLoading[request._id]}
                        >
                          <FontAwesomeIcon icon={faUserTimes} className="mr-1 h-3 w-3" /> 
            ...
            <div className="bg-white">
              <h2 className="text-lg sm:text-xl font-semibold mb-2 text-purple-800">Your Connections</h2>
              {connections?.length === 0 ? (
                <p className="text-xs text-gray-600">You don't have any connections yet.</p>
              ) : (
                <ul className="divide-y divide-gray-200">
                    ...
                    <li key={connection._id} className="py-2 flex items-start">
                      ...
                      <div className="flex-grow">
                        <h3 
                          ...
                          onClick={() => navigate(`/connection/profile/${connection._id}`)}
                        >
                          {connection.name}
                        </h3>
                        <p className="text-xs text-gray-600 mb-1">{truncateBio(connection.bio)}</p>
                        <div className="flex flex-wrap gap-1 text-xs text-gray-500">
                          {connection.email && (
                            <span className="flex items-center">
                              <FontAwesomeIcon icon={faEnvelope} className="mr-1 h-3 w-3" />
                              {connection.email}
                            </span>
                          )}
                          {connection.phone && (
                          ...
                          {connection.experiences && connection.experiences.length > 0 && (
                            <span className="flex items-center">
                              <FontAwesomeIcon icon={faBriefcase} className="mr-1 h-3 w-3" />
                          ...
                          {connection.gender && (
                            <span className="flex items-center">
                              <FontAwesomeIcon icon={faVenusMars} className="mr-1 h-3 w-3" />
                              {connection.gender}
                            </span>
                          )}
                        </div>
                        {connection.skills && connection.skills.length > 0 && (
                          <div className="mt-1">
                            <h4 className="text-xs font-semibold text-gray-700">Skills:</h4>
                            <div className="flex flex-wrap gap-1 mt-1">
                              {connection.skills.map((skill, index) => (
                                <span key={`${connection._id}-skill-${index}`} className="bg-purple-100 text-purple-800 text-xs px-1 py-0.5 rounded">
                                  {skill}
                                </span>
                              ))}
                            </div>
                      ...
                      <button className="ml-2 bg-purple-100 text-purple-700 px-2 py-1 rounded-full hover:bg-purple-200 transition duration-300 text-xs flex items-center">
                        <FontAwesomeIcon icon={faUserCheck} className="mr-1 h-3 w-3" /> Connected
                      </button>
                      <button 
                      ...
                      >
                        <FontAwesomeIcon icon={faUserTimes} className="mr-1 h-3 w-3" /> Remove
                      </button>
                    </li>
                  ))} 
                </ul>

...
export class ProfileController {
  ...
  async updateUserProfile(req: Request, res: Response) {
    try {
      ...
      const updatedProfile = await this.updateUserProfileUseCase.execute(userId, profileData);
  ...
  async updateUserProjects(req: Request, res: Response) {
    try {
      ...
      const updatedProfile = await this.updateUserProfileUseCase.execute(userId, { projects });
      res.status(200).json(updatedProfile);
    } catch (error) {
  ...
  async updateUserCertificates(req: Request, res: Response) {
    try {
      const userId = req.params.userId;
      const files = req.files?.['certificateImage'] as Express.Multer.File[] || [];
      ...
      const certificates = certificateDetails.map((details: any, index: number) => {
        const file = files[index]; // Map the correct file for each certificate
        return {
          ...details,
          imageUrl: file ? `/uploads/user/certificates/${file.filename}` : details.imageUrl,
        };
      });
  
      const updatedProfile = await this.updateUserProfileUseCase.execute(userId, { certificates });
  ...
  async updateUserExperiences(req: Request, res: Response) {
    try {
      ...
      const updatedProfile = await this.updateUserProfileUseCase.execute(userId, { experiences });
      res.status(200).json(updatedProfile);
    } catch (error) {
  ...
  async updateUserResume(req: Request, res: Response) {
    try {
      ...
      const updatedProfile = await this.updateUserProfileUseCase.execute(userId, { resume: resumePath });
      res.status(200).json(updatedProfile);
    } catch (error) {

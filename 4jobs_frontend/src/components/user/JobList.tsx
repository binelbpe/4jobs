...
import { fetchJobPostsAsync, applyForJobAsync, fetchJobPost } from '../../redux/slices/authSlice';
...
const JobList: React.FC = () => {
  ...
  const handleApply = async (jobId: string) => {
    if (user?.id) {
      try {
        const resultAction = await dispatch(applyForJobAsync({ userId: user.id, jobId }));
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="space-y-6">
          ...
          {posts.map((job: BasicJobPost) => (
            <div key={job._id} className="bg-white shadow-lg rounded-lg overflow-hidden transition-all duration-300 hover:shadow-xl">
              <div className="p-6">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4">
                  <h2 className="text-xl font-semibold text-gray-800 mb-2 md:mb-0">{job.title}</h2>
        ...
        <p className="mt-6 text-center text-gray-600">Total Jobs: {totalCount}</p>
      </div>
    </div>
  );
};

...
export class ConnectionController {
  ...
  async sendConnectionRequest(req: Request, res: Response): Promise<void> {
    try {
      ...
      await this.createAndEmitNotification('connection_request', senderId, recipientId, connection.id);
  ...
  async getConnectionRequests(req: Request, res: Response): Promise<void> {
    try {
      const userId = req.params.userId;
      const requests = await this.connectionUseCase.getConnectionRequests(userId);
      res.json(requests);
    } catch (error) {
  ...
  async acceptConnectionRequest(req: Request, res: Response): Promise<void> {
    try {
      ...
      const connection = await this.connectionUseCase.acceptConnectionRequest(connectionId);
      await this.createAndEmitNotification('connection_accepted', userId, connection.requesterId, connection.id);
  ...
  async rejectConnectionRequest(req: Request, res: Response): Promise<void> {
    try {
      const { connectionId } = req.params;
      const { userId } = req.body;
      const connection = await this.connectionUseCase.rejectConnectionRequest(connectionId);
  ...
  private async createAndEmitNotification(type: string, senderId: string, recipientId: string, relatedItemId: string): Promise<void> {
  ...
  async searchConnections(req: Request, res: Response) {
    try {
      ...
      const result = await this.connectionUseCase.searchConnections(userId, query);
      res.json(result);
    } catch (error) {
  ...
  async searchMessageConnections(req: Request, res: Response): Promise<void> {
    try {
      const userId = req.params.userId;
      const query = req.query.query as string;
      const results = await this.connectionUseCase.searchMessageConnections(userId, query);
      res.json(results);
    } catch (error) {
